...
class UI(Form, Base):
    ...
    def setEpPath(self):
        filename = QFileDialog.getExistingDirectory(self, self.title, self.lastPath, QFileDialog.DontUseNativeDialog)
        if filename:
    ...
    def populate(self):
        if not os.environ['USERNAME'] in ['qurban.ali', 'sarmad.mushtaq', 'mohammad.bilal', 'talha.ahmed', 'assad.siddiqui', 'salman.rauf']:
        ...
        if not osp.exists(seqsDir):
            self.showMessage(msg='The system could not find the SEQUENCES directory',
                             icon=QMessageBox.Critical)
            return
        seqs = [seq for seq in os.listdir(seqsDir) if re.match('^SQ\d{3}$', seq)]
        if not seqs:
            self.showMessage(msg='The system could not find the Sequences directory',
                             icon=QMessageBox.Critical)
            return
        ...
        for seq in seqs:
            ...
            if not osp.exists(shotsDir): errors['SHOTS directory not found in %s'%seq] = ''; continue
